...
## 2.31.7 (2025-10-03) - ygqygq2 Extended

- [ygqygq2/common] Extended with Gateway API support (based on bitnami/common 2.31.7)
  - Add `_gateway.tpl` with Gateway API related helpers
  - Support HTTPRoute, GRPCRoute, TCPRoute, TLSRoute naming helpers
  - Support Backend, BackendTrafficPolicy, ClientTrafficPolicy naming helpers
  - Add `common.gateway.parentRefs` for generating standard parentRefs structure
  - Add `common.gateway.processBackendRefs` for auto-filling empty name/port in backendRefs
  - Add route-specific rule helpers with smart defaults (httproute.rules, grpcroute.rules, etc.)
  - Add BackendTrafficPolicy and Backend spec generators supporting both full spec mode and legacy field-by-field mode
  - All helpers designed to be modular and easy to merge with future Bitnami updates
